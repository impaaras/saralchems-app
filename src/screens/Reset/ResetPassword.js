...
export default function ResetPassword({navigation}) {
  ...
  const responsiveStyles = StyleSheet.create({
    ...
    logoContainer: {
      alignItems: 'center',
      marginTop: isSmallDevice ? hp(12) : hp(12),
  ...
  const getInputStyle = () => {
    if (errors.email) return responsiveStyles.inputError;
    if (email.length > 0 && /^\S+@\S+\.\S+$/.test(email))
      return responsiveStyles.inputValid;
    if (email.length === 0) return responsiveStyles.inputEmpty;
  ...
  // Handle reset password with validation
